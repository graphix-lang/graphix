...
- Fleet: katana still holds an unpulled **sep03e** (18 divergences,
  2026-09-03) from the A2 "sleep deleted" tree. Main was reset off that
  line when mux-select was withdrawn. Eric's call (2026-09-06): a
  different engine, almost certainly not relevant any more — leave them
  unless that arc is revived.
...
| sep01b | 2026-09-01 | 0 | — | ~3h on the strict-default flip (fa08136a), five remotes; closed clean and superseded same day by the deletion commits (41290a01, a6241025) — sep01c soaks the tree with the stateful-kernel machinery gone |
| sep01c | 2026-09-01 | 1 | 1 | ~1.5h on the deletion tree, five remotes. ryouko 00: SELECT WAKE RE-MATCH (`select-wake-rematch-sep2026`) — an interp bug in wake catch-up, the JIT was right: a select's first update after sleep rode the selection it retained against a scrutinee value that had moved while no reader was awake (an arm-local `<-` counter; no tracked fire exists for an arm-local bind). Select now owns a `slept` bit like every other node and its wake re-matches. Predates the deletion (the kernel side is unchanged) |
| sep01d | 2026-09-01 | 0 | — | ~25 min on the `?` delivery-queue tree (032e3338), five remotes; superseded by the select wake re-match fix (bc854828) — sep01e soaks it |
| sep01e | 2026-09-02 | 0 | — | ~2h on the select wake re-match tree (bc854828), five remotes — wake catch-up's quiet round, 0 divergences at every status check; superseded by the fastcall growth sweep (30547a7a: typed fastcall, default-label fill, null args, `FastMemo`, 20 conversions) — sep02a soaks it (base 7700M) |
| sep02a | 2026-09-03 | 4 | 1 | ~19h on the fastcall-sweep tree (30547a7a), five remotes; hz0 alone found four CRASHES (two outer-deadline hangs, a stack-budget abort, an OOM), one class, ALL AT COMPILE TIME: `contains` on two instantiations of the recursive `ErrChain` alias (`catch(e) err1 <- e` with `err1` declared over a different error union than the catch delivers — an ill-typed program) looped — the Ref×Ref cycle memo is dropped once the direct walk fails, and the coverage-distribution probe (08-31) re-expanded the alias through `lookup_ref` with no memo of its own, re-asking the failing `cause`-field question one level deeper per round. Fixed: the probe keeps its in-progress scrutinee ids on `RefHist` and a re-entrant probe claims nothing (`errchain-distribution-loop-sep2026`, `lang/errors.rs` `catch_connect_union_mismatch_is_an_error`). All four now report the type mismatch |
| sep04a | 2026-09-06 | 3 | 1 | ~26h on the straight-line-`seq` tree (13323013), washu-chan alone (the rest of the fleet was down). One symptom — a `<-` target whose initializer stands bottom loses its write — from TWO emitter bugs, each minting a FRESH bottom on a quiet cycle where the node-walk mints a standing one: the tail-position select's tainted-scrutinee return used `emit_elem_placeholder` raw (its value-position and UNDETERMINED twins already OR the scrutinee's STALE bit), and `SlotFlags::exact_stale` computed `resized` against the over-limit CLAMPED length while holding the prev-length word, so every quiet over-limit cycle read as a resize. Pinned `standing-bottom-refire-sep2026/00-02`, attributed per fix by half-reverted builds. Follow-up audit the same day: 14 probes over every placeholder-consuming path all agree on HEAD (9 of them go red against a build with every placeholder STALE fold removed, so the green is evidence), and the API that generated the bug is closed — `emit_bottom_placeholder(cx, typ, fires)` folds the governing tags itself |
