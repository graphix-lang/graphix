...
| `pure_select.md` | select arms are PURE, sleep disappears (Eric's alternative, 2026-09-03): impure code in an arm is sugar for a hoisted, arm-gated form (a gated level is born at entry, a gated occurrence is not); level effects follow presence; deletes wake catch-up, Held, the restart contracts, 115 sleep impls; go/no-go = the hoist as a printer over the port |
