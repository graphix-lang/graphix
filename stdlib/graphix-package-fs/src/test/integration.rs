...
const WRITE_THEN_READ: &str = r#"{
  ...
  let path = fs::join_path(fs::tempdir::path(temp), "write_read_test.txt");
...
const WRITE_THEN_WATCH_MODIFY: &str = r#"{
  let paths = {
    let temp = fs::tempdir::create(null)?;
    let temp_path = fs::tempdir::path(temp);
    let file_path = fs::join_path(temp_path, "watch_write_test.txt");
...
const WRITE_BIN_THEN_READ_BIN: &str = r#"{
  let temp = fs::tempdir::create(null)?;
  let path = fs::join_path(fs::tempdir::path(temp), "binary_cycle.bin");
