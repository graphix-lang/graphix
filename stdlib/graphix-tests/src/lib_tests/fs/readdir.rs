...
run_with_tempdir! {
    ...
    code: r#"sys::fs::readdir(#min_depth: 2, #max_depth: 3, "{}")"#,
    setup: |temp_dir| {
        let root = temp_dir.path();
        fs::write(root.join("root.txt"), "root").await?;
        fs::create_dir(root.join("sub1")).await?;
...
run_with_tempdir! {
    ...
    code: r#"sys::fs::readdir(#max_depth: 2, #follow_symlinks: true, "{}")"#,
    setup: |temp_dir| {
        let root = temp_dir.path();
...
run_with_tempdir! {
    ...
    code: r#"sys::fs::readdir(#max_depth: 2, #follow_symlinks: false, "{}")"#,
    setup: |temp_dir| {
        let root = temp_dir.path();
        fs::create_dir(root.join("real_dir")).await?;
        fs::write(root.join("real_dir").join("file.txt"), "content").await?;
...
run_with_tempdir! {
    ...
    code: r#"sys::fs::readdir(#follow_root_symlink: false, "{}")"#,
    setup: |temp_dir| {
        let root = temp_dir.path();
        fs::create_dir(root.join("real_dir")).await?;
        fs::write(root.join("real_dir").join("file.txt"), "content").await?;
...
run_with_tempdir! {
    ...
    code: r#"sys::fs::readdir(#max_depth: 1, #min_depth: 2, "{}")"#,
    setup: |temp_dir| {
        temp_dir.path().to_path_buf()
    },
    expect_error
...
run_with_tempdir! {
    ...
    code: r#"sys::fs::readdir(#max_depth: -1, "{}")"#,
    setup: |temp_dir| {
        temp_dir.path().to_path_buf()
    },
    expect_error
...
run_with_tempdir! {
    ...
    code: r#"sys::fs::readdir(#min_depth: -1, "{}")"#,
    setup: |temp_dir| {
        temp_dir.path().to_path_buf()
    },
    expect_error
