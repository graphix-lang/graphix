//! Seed corpus for the mutation engine: hand-written expressions biased
//! toward the shapes where fusion bugs live, plus the harvested
//! graphix-tests fixtures. Each entry is an expression the fuzzer wraps
//! as `let result = {seed}`.

// Generated by build.rs: `pub static HARVESTED: &[&str]`.
include!(concat!(env!("OUT_DIR"), "/harvested_seeds.rs"));

// Generated by build.rs: `pub static REGRESSION_CORPUS: &[(&str, &str)]`
// as (relative name, program).
include!(concat!(env!("OUT_DIR"), "/regression_corpus.rs"));

/// Hand seeds plus the harvested fixture corpus.
...
pub const SEEDS: &[&str] = &[
    ...
    // scheduled reactive seeds: a `// schedule-v1:` header plus a body
    // over the driver-declared inputs
    ...
    // cross-cycle builtins over an injected source
