...
runs twice. `on_press(e, f)` calls `f` with the `KeyEvent` when `e` is a
press and continues on everything else. It returns what a handler
returns, so it is usually the handler's whole body:
