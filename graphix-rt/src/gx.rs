...
impl<X: GXExt> GX<X> {
    ...
    async fn check(
        ...
        self.check_inner(source, resolver_override).await.map(|(_, r)| r)
    }
    ...
    async fn check_and_capture(
        &mut self,
        source: &Source,
    ) -> Result<(Arc<[Expr]>, FxHashMap<ExprId, graphix_compiler::typ::FnType>)> {
        let prior_fn_types = std::mem::take(&mut self.ctx.fn_types);
        match self.check_inner(source, None).await {
            Ok((exprs, _)) => {
                // fn_types was populated by Lambda::compile +
                // CallSite::typecheck during check_inner. Snapshot it,
                // then restore prior so ctx isn't left polluted.
