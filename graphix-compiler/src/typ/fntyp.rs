...
impl Walking {
    fn enter(set: &'static LocalKey<RefCell<IntSet<usize>>>, ft: &FnType) -> Option<Self> {
...
impl FnType {
    ...
    fn constraint_pairs(&self, keep: impl Fn(&TVar) -> bool) -> LPooled<Vec<(TVar, Type)>> {
...
impl FnType {
    ...
    pub fn sig_matches(&self, env: &Env, impl_fn: &Self) -> Result<()> {
        self.sig_matches_int(env, impl_fn, &mut LPooled::take(), &mut super::RefHist::new())
    }
