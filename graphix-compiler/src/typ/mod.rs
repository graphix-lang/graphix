...
struct RefHist<H: IsoPoolable> {
    ...
    ref_ids: LPooled<IntMap<usize, (Arc<ResolvedRef>, SmallVec<[(Arc<[Type]>, usize); 2]>)>>,
    ...
    fn ref_id(&mut self, t: &Type, env: &Env) -> Option<usize> {
        ...
        let k = match t {
            Type::Ref(tr) => {
                ...
                let key = r.def_key();
                let entry = self.ref_ids.entry(key).or_insert_with(|| (r, SmallVec::new()));
                ...
                let id = self.next();
                self.ref_ids.get_mut(&key).expect("inserted").1.push((tr.params.clone(), id));
                return Some(id);
            }
            ...
            Type::Abstract { id, params } => (d, (**params).as_ptr().addr(), id.0 as usize),
    ...
    pub(crate) fn resolve_pure(&self, env: &Env) -> Option<Arc<ResolvedRef>> {
        env.resolve_visible(&self.scope, &self.name, crate::env::NameNs::Type, |s, n| {
            env.typedefs.get(s).and_then(|m| m.get(n)).map(|d| {
                Arc::new(ResolvedRef {
                    canonical_scope: ModPath(netidx_core::path::Path::from(ArcStr::from(s))),
                    pos: d.pos,
                    ori: d.ori.clone(),
...
impl Type {
    ...
    fn cmp_fields(&self, other: &Self) -> Ordering {
        match (self, other) {
            ...
            (Type::Abstract { id: i0, params: p0 }, Type::Abstract { id: i1, params: p1 }) => {
...
impl Drop for Type {
    fn drop(&mut self) {
        ...
        let last = match self {
            Type::Bottom | Type::Any | Type::Hole | Type::Primitive(_) | Type::TVar(_) => {
                false
            }
            ...
            Type::Error(a) | Type::Array(a) | Type::List(a) | Type::ByRef(a) => a.is_unique(),
            Type::Struct(fs) => fs.is_unique(),
            Type::Map { key: a, value: b } | Type::App(a, b) => a.is_unique() || b.is_unique(),
        };
        if !last {
            return;
        }
...
impl Type {
    ...
    pub(crate) fn app_split_for(
        ...
        for c in cv.cell_constraints().iter() {
            ...
            for im in heads.iter() {
                if !matches!(im.target, Type::Ref(_)) || fits(&im.target, &t.reset_tvars())?.is_none() {
                    continue;
                }
        ...
        fn go(t: &Type, env: &Env, def: (&str, &str), depth: usize) -> bool {
            ensure_sufficient(|| match t {
                ...
                Type::Ref(tr) if depth < UNGUARDED_DEPTH => {
                    ...
                    let base = netidx_core::path::Path::basename(&*tr.name).unwrap_or(&tr.name);
    ...
    pub fn lookup_ref(&self, env: &Env) -> Result<Type> {
        Ok(self.lookup_ref_with(env, true)?.expect("a committing lookup refuses by error"))
    }
    ...
    pub(crate) fn lookup_ref_with(&self, env: &Env, commit: bool) -> Result<Option<Type>> {
        match self {
            Self::Ref(tr) => {
                ...
                for ((_, constraint), arg) in def_params.iter().zip(params.iter()) {
                    ...
                    match arg {
                        ...
                        _ => {
                            if !constraint.contains_with_flags(BitFlags::empty(), env, arg)? {
                                return Ok(None);
                            }
                        }
                    }
            ...
            Type::Ref(tr) => {
                if let (Some(pos), Some(ori)) = (tr.pos, &tr.ori) {
                    let (canonical_scope, def_pos, def_ori) = match tr.resolve_pure(env) {
                        ...
                        None => (fallback_scope.clone(), SourcePosition::default(), ori.clone()),
                    };
                    env.push_type_ref(crate::ide::TypeRefSite {
                        pos,
                        ori: ori.clone(),
