...
impl FusionCtx {
    ...
    /// kernel, and forget the previous check's kernel signatures and
    /// fusion outcomes: every lambda id they key on died with it.
    ///
    ...
    /// # Safety
    ///
    /// No [`FusedKernel`] this context compiled may run afterwards: its
    /// code belongs to the module this discards.
    pub unsafe fn reset_jit_for_check(&mut self) -> anyhow::Result<()> {
        *self.jit.lock() = None;
...
pub(crate) struct Discovery<'n, R: Rt, E: UserEvent> {
    ...
    /// processes.
    pub(crate) callees: LPooled<Vec<(usize, Arc<KernelSig>)>>,
    /// Each callee's body, self-call info and own call sites, by kernel
    /// identity.
    pub(crate) bodies: BTreeMap<usize, CalleeBody<'n, R, E>>,
...
pub fn try_fuse<R: Rt, E: UserEvent>(
        ...
        && arena_exhausted(e)
    {
        match emit::Jit::new() {
            Ok(fresh) => {
                let old = ctx.fusion.jit.lock().replace(fresh);
                ctx.fusion.retired_jits.lock().extend(old);
...
fn arena_exhausted(e: &anyhow::Error) -> bool {
    use cranelift_module::ModuleError;
    e.chain().any(|c| {
        matches!(c.downcast_ref::<ModuleError>(), Some(ModuleError::Allocation { .. }))
    })
}
