...
/// An error nothing handles, or a hot operator's failure: logged, and
/// written to stderr, which a shell without a log still shows.
pub(crate) fn report_failure(msg: &str) {
    log::error!("{msg}");
    eprintln!("{msg}");
}

/// A `$` at `site` dropping the error payload `e`.
pub(crate) fn report_ignored(site: &str, e: &dyn fmt::Display) {
