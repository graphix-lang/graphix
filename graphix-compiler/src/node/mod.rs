use crate::{
    ...
    expr::{At, Expr, ExprId, ExprKind, ModPath, Name, Origin, TypeDefBody, UseItem},
    fusion::{
        emit::{
...
pub(crate) fn compile_block_children<'a, R: Rt, E: UserEvent>(
    ...
    for (i, e) in exprs.iter().copied().enumerate() {
        ...
        let (node, next) =
            compile_statement(ctx, flags, e, &scope, top_id, value_position)?;
        scope = next;
        children.push(node);
    }
...
/// `catch` covers. In a `do` block's value slot a declaration compiles
/// as an expression, which refuses it.
pub(crate) fn compile_statement<R: Rt, E: UserEvent>(
    ...
    let node = match &e.kind {
        ...
        _ if value_position => compile(ctx, flags, e.clone(), scope, top_id),
