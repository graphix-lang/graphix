...
macro_rules! trait_decl {
    ($inner:expr, $doc:expr) => {
        (
            typart(),
            collection::vec(
                (random_fname(), trait_method_sig(), option::of($inner), $doc, comments()),
                0..4,
            ),
        )
...
fn undecorated_sigitem() -> impl Strategy<Value = SigItem> {
    prop_oneof![
        ...
        (trait_decl!(constant(), option::of(arcstr())), option::of(arcstr())).prop_map(|(mut t, doc)| {
