    ...
    I::Range: Range,
{
    (
        ...
        .then(
            |((at, name), mut methods, end): ((_, ArcStr), LPooled<Vec<TraitMethod>>, _)| {
    ...
    (
        ...
        .then(
            |(params, at_params, trait_name, target, methods, end): (
                ...
                let tvs: Arc<[TVar]> = Arc::from_iter(params.iter().map(|(tv, _)| tv.clone()));
                if tvs.iter().enumerate().any(|(i, tv)| tvs[..i].iter().any(|p| p.name == tv.name))
                {
                ...
                if !ms.iter().all(simple) {
                    return refuse(end, "an impl body holds only `let name = ..` methods")
                        .right();
                }
                value(ImplExpr {
                    trait_name,
