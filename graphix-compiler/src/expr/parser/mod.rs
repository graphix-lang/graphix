...
use netidx_value::{
    Value,
    parser::{VAL_ESC, VAL_MUST_ESC, not_prefix, sep_by_tok, value as parse_value},
};
...
where
    ...
    F: Fn(I::Position) -> O,
{
    sep_by1((position(), choice((ahead(term).map(|_| None::<O>), p.map(Some)))), sep).map(
        move |mut e: LPooled<Vec<(_, Option<O>)>>| {
            let mut res = OC::default();
    ...
    I::Range: Range,
{
    (
        position(),
        attempt((token('r'), many::<CompactString, _, _>(token('#')), token('"'))),
    )
    ...
    I::Range: Range,
{
    ...
    attempt(
        spaces().skip(not_followed_by(token('{'))).skip(not_followed_by(clause)).with(
            choice((
                (letbind_with(arithexp::arith(false)), position()).then(
                    |(b, pos)| match b.rec {
                        true => grow::refuse(
                            pos,
                            "a seq trigger cannot be rec: it has no self to recurse on",
                        )
                        .right(),
    ...
    I::Range: Range,
{
    optional(seq_trigger()).then(move |trigger| {
        ...
        optional(seq_clause("abort", first)).then(move |abort| {
            ...
            (optional(flush), position(), optional(attempt(sptoken(';'))), position())
                .then(move |(flush, at_flush, semi, at_semi)| {
                    if semi.is_some() {
                        grow::refuse(
                            at_semi,
    ...
    I::Range: Range,
{
    ...
    (sep_by1_tok(field, csep(), token('}')), position()).and_then(
        |(mut fields, end): (LPooled<Vec<(ArcStr, Expr)>>, _)| match sort_unique(
            &mut fields,
            |(n, _)| n,
        ) {
            true => Ok(fields),
            ...
            .then(|(pos, body, end, _)| {
                let kind = match body {
                    ...
                    Some((k, Tail::Map(v, mut rest))) => ExprKind::Map {
                        args: Arc::from_iter(
                            std::iter::once((k, v)).chain(rest.drain(..)),
                        ),
                    },
                    Some((source, Tail::With(mut fields))) => {
                        match with_source(&source) {
                            None => {
                                return grow::refuse(
                                end,
                    ...
                    Some((_, Tail::One)) => {
                        return grow::refuse(
                            ...
                            "a block must contain at least 2 expressions",
                        )
                        .right();
                    }
                };
