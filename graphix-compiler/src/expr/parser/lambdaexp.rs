    ...
    I::Range: Range,
{
    ...
    (sep_by_tok(arg, csep(), attempt(sptoken('|'))), position()).and_then(
        |(mut v, end): (LPooled<Vec<LambdaArg>>, _)| {
            ...
            let mut vargs = None;
            for (i, ((pos, (labeled, pattern)), constraint, default)) in v.drain(..).enumerate()
            {
                ...
                match &pattern {
                    StructurePattern::Bind(b) if b == "@args" => {
                        if i + 1 < n {
                            return Err(refusal::<I>(end, "@args must be the last argument"));
                        }
    ...
    I::Range: Range,
{
    (
        position(),
        spaces()
            .with(sep_by_tok(tvar_bound(), csep(), token('|')))
            .map(|mut tvs: LPooled<Vec<(TVar, LPooled<Vec<Type>>)>>| {
