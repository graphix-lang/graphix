...
; Anonymous functions — tag only the parameter delimiters; tagging the
; whole node would paint the body in function color.
...
; (`labeled_param` exists in the grammar but is unreachable, so it isn't
; an emitted node type — don't reference it here.)
(labeled_param_with_default) @variable.parameter
