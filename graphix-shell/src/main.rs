...
fn handle_compile(
    ...
    // parsed `Expr` tree and an IntMap<ExprId, FnType>. Fusion
...
fn run_pre_fusion_typecheck(
    ...
    rt.block_on(async move {
        ...
        let mut vfs_modules = ahash::AHashMap::default();
...
fn graphix_shell_deps_register<X: graphix_rt::GXExt>(
    ...
    >,
    modules: &mut ahash::AHashMap<netidx::path::Path, arcstr::ArcStr>,
