...
impl<X: GXExt> Tui<X> {
    pub(crate) fn start(gx: &GXHandle<X>, env: Env, root: CompExp<X>) -> Self {
        let gx = gx.clone();
    ...
    pub(crate) async fn update(&mut self, id: ExprId, v: Value) {
        if let Err(_) = self.to.send(ToTui::Update(id, v)).await {
            error!("could not send update because tui task died")
        }
    }

    pub(crate) async fn wait_signal(&mut self) -> Signal {
